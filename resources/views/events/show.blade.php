            ...
            <div id="info-container" class="col-md-6">
                ...
                @if(!$hasUserJoined)
                    <form action="/events/join/{{$event->id}}" method="POST">
                        @csrf
                        <a href="event/join/{{$event->id}}" class="btn btn-primary"
                        id="event-submit" onclick="event.preventDefault();this.closest('form').submit();" id="event-submit">Confirmar presença</a>
                    </form>
                @else
